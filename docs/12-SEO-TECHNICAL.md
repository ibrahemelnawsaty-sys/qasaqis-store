...
### لماذا تُحسب og:title/og:description في PHP لا في السمة
...
> ⚠️ **فخّ Blade — لا تعد كتابتها كتوجيهات داخل السمة.**
> المحاولة الأولى كانت:
> ```blade
> content="@hasSection('og_title')@yield('og_title')@else@yield('title', …)@endif"
> ```
> وهي **مكسورة**: Blade يطابق التوجيهات بـ `\B@`، فأي توجيه يلاصق **حرف كلمة** لا
> يُتعرَّف عليه. في `@else@yield(...)` يسبق الـ `@` الحرفُ `e`، فيفشل `\B` ويبقى
> `@yield('title', …)` نصًّا خامًا **يُطبع حرفيًا داخل الوسم**. لم يظهر العطل في
> `php -l` ولا في `view:cache` — ظهر فقط عند **تصيير الصفحة وقراءة الناتج**.
>
> الشكل الصحيح المعتمد: حساب `$seoOgTitle`/`$seoOgDescription` في كتلة `@php` قبل
> الوسوم، ثم طباعتها بـ `{{ }}` — وهذا يُهرِّب المحتوى أيضًا، بخلاف `@yield`.

---

## 1.5 التعبئة التلقائية لبيانات SEO — محرّك `SeoDefaults`

**المبدأ:** الأدمن لا يكتب ميتا لكل صفحة. ما يُترك فارغًا يُشتقّ تلقائيًا من محتوى
الكيان نفسه، ويظلّ محدَّثًا دائمًا (لا نصّ مجمَّد في قاعدة البيانات).

### المصدر الواحد

`app/Support/Seo/SeoDefaults.php` يشتقّ العنوان/الوصف من حقول المحتوى فقط (لا يقرأ
`seo_meta`). يُستدعى من **موضعين ليتطابقا**:

1. **القوالب (render):** الطرف المشتقّ في سلسلة «التجاوز المخزّن `?:` المشتقّ».
   ```php
   ...
   ```
   مطبَّق في: `books/show`، `pages/show`، `blog/show`، و`catalog/index` (قسم/سلسلة).
2. **Filament (placeholder):** `app/Filament/Support/SeoPlaceholder.php` يعرض القيمة
   المشتقّة نفسها كـ placeholder رمادي، فيرى الأدمن ما سيُنشر إن ترك الحقل فارغًا.

### قواعد الاشتقاق (لكل نوع)

| النوع | العنوان | الوصف |
|---|---|---|
| Book | `{title} — {brand}` | `short_description` ← نصّ `long_description` ← الشعار |
| Page | `{title} — {brand}` | نصّ `content` ← الشعار |
| Article | `{title} — {brand}` | `excerpt` ← نصّ `content` ← الشعار |
| Category | `كتب {name} للأطفال — {brand}` | `description` ← قالب `seo.category_description` |
| Series | `سلسلة {name} — {brand}` | `description` ← قالب `seo.series_description` |

قواعد ثابتة: اسم العلامة يُلحَق **مرّة واحدة** (لا يتكرّر إن كان العنوان يحويه)؛ HTML
يُستبدل بمسافات (لا `strip_tags` وحده، وإلا التصقت الكلمات عبر حدود العناصر)؛ الوصف
يُقتطع عند **160 حرفًا** (حدّ مقتطف البحث). النصوص العربية في `lang/ar/seo.php` (بند 6.4).

### أين تُخزَّن التجاوزات (٣ أنماط)

- **`seo_meta` polymorphic:** Book، Page، Category، **Series** (أُضيفت `seo()` للسلسلة
  على الجدول المشترك بلا هجرة). واجهتها في Filament: قسم `->relationship('seo')`.
  Book/Page لهما قسمهما الأوفى؛ Category/Series يستخدمان المصنع المشترك
  `app/Filament/Support/SeoFieldset.php`.
- **أعمدة مباشرة:** Article (`articles.seo_title` / `seo_description`).
- **الرئيسية:** وصفها من إعداد CMS (`hero_subtitle`)، لا من هذا المحرّك.

> **قبل هذا العمل:** Category كانت لها علاقة `seo()` **ميتة** (بلا واجهة)، وSeries بلا
> SEO إطلاقًا، و`catalog/index` يحسب `$series` **ولا يستخدمه** فصفحات السلاسل بلا وصف
> مشتقّ. الثلاثة أُصلحت.

### التحقّق

`tests/Feature/Seo/SeoDefaultsTest.php` — ١١ اختبارًا (٢٠ تأكيدًا)، بلا قاعدة بيانات
(نماذج غير مخزّنة)، تمرّ عبر `php artisan test`. تغطّي: إلحاق العلامة مرّة، تفضيل
الوصف القصير/المقتطف، تنظيف HTML وفصل الكلمات، الاقتطاع، والرجوع للشعار.
...
| البند | الحالة |
|---|---|
| `php -l` على كل ملف PHP وكل كتلة Blade | ✅ نظيف |
| `php artisan view:cache` — تصريف **كل** قوالب التطبيق | ✅ نجح |
| تصيير الـ layout فعليًا وقراءة الـ HTML الناتج | ✅ **هذا ما كشف عطل `@hasSection`** — كل الوسوم مفردة (canonical/robots/og:\* = ١)، والـ JSON-LD صالح، والتجاوزات تعمل |
| اختبارات تشغيل لمولّدات JSON-LD الثلاثة | ✅ ترشيح `null`، ترقيم المواضع، الحارس عند غياب مفتاح الترجمة |
| `php artisan test` | ❌ **لم يُشغَّل** — يتطلّب MySQL وقاعدة `qasaqis_test`؛ سير العمل المعتمد يشغّلها على الخادم. |
| تصيير صفحات حقيقية بمحتوى من قاعدة البيانات | ❌ **لم يُشغَّل** — SQLite لا يدعم `fullText` في هجرة الكتب. |
...
> **الدرس:** `php -l` و`view:cache` **لا يكفيان**. عطل `@hasSection` اجتازهما معًا
> ولم ينكشف إلا بتصيير الصفحة وقراءة الـ HTML. أي تعديل على وسوم الـ `<head>` يجب
> أن يُختبر بالتصيير الفعلي.
